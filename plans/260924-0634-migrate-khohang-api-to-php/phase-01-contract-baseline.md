...
- Chốt framework PHP, giới hạn thay đổi client, cách bảo toàn JWT đang còn hiệu lực; ghi nhận quyết định giữ FIFO hiện tại trong migration. `calculate_fifo_cost()` duyệt các phiếu nhập nhưng không trừ lượng đã xuất trước đó, nên việc sửa thuộc thay đổi riêng.
...
4. Compare Laravel + compatible WS server against Laravel Reverb and framework-free PHP; record the accepted direction and its failing assumption. Giữ FIFO cũ trong bản port và lập thay đổi nghiệp vụ riêng cho defect đã nhận diện.
...
- [ ] Architecture được chốt; quyết định giữ FIFO hiện tại và security gap về passkey được ghi rõ.
